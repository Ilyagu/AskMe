...
class AskForm(forms.ModelForm):
    ...
    def save(self, **kwargs):
        question = Question()
        question.author = self._profile_id
...
class AnswerForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "text": Textarea(attrs={"class": "form-control", "rows": "2", "placeholder": "Enter your answer here..."}),
        }
    ...
    def save(self, **kwargs):
        self.cleaned_data["author"] = self._profile_id
...
class SignupForm(forms.ModelForm):
    ...
    ava = forms.FileField(required=False, label="Avatar")
...
class LikeQuestionForm:
    ...
    def save(self):
        if not LikeQuestion.objects.filter(question=self.question, user=self.user).exists():
            like = LikeQuestion(question=self.question, user=self.user, opinion=self.is_like)
            rating = like.save()
        else:
            like = LikeQuestion.objects.get(question=self.question, user=self.user)
...
class LikeAnswerForm:
    ...
    def save(self):
        if not LikeAnswer.objects.filter(answer=self.answer, user=self.user).exists():
            like = LikeAnswer(answer=self.answer, user=self.user, opinion=self.is_like)
            rating = like.save()
        else:
            like = LikeAnswer.objects.get(answer=self.answer, user=self.user)
            if self.is_like == like.opinion:
                rating = like.delete()
            else:
                rating = like.change_mind()
