...
def tag(request, name):
    _tag = get_object_or_404(Tag, name=name)
    ...
    return render(
        request,
        "tag.html",
        {"tag": _tag, "questions": questions, "popular_tags": popular_tags, "best_members": best_members},
    )
...
def answers_for_question(request, pk):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            form.save()
            answers_page = paginate(Answer.objects.by_question(pk), request, per_page=1)
            return redirect(
                reverse("answers_for_question", kwargs={"pk": pk}) + f"?page={answers_page.paginator.num_pages}"
            )
...
def new_question(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            question = form.save()
            return redirect(reverse("answers_for_question", kwargs={"pk": question.pk}))
    return render(
        request,
        "new_question.html",
        {"form": form, "popular_tags": popular_tags, "best_members": best_members},
    )
